...
import { toggleFavorite, fetchFavoriteIds, getBoutiqueUrl } from "@/lib/catalogue";
import { motion, AnimatePresence, Variants } from "framer-motion";
import { HomeSkeleton } from "@/components/ui/Skeleton";
import type { BentoData } from "@/lib/queries/bento";
...
export default function Home() {
  ...
  // Bento dynamique — appel séparé et non bloquant : les données viennent
  // d'une route cachée côté serveur (1h, voir lib/queries/bento.ts), donc
  // rapide, mais on ne veut pas qu'un souci ici retarde le reste de la home.
  useEffect(() => {
    let cancelled = false;
    fetch("/api/bento")
      .then((res) => res.json())
      ...
      .catch((err) => console.error("Erreur chargement bento:", err))
      .finally(() => {
        if (!cancelled) setBentoLoading(false);
      });
    return () => { cancelled = true; };
  }, []);
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans antialiased">
      ...
      {dataLoading ? (
        ...
        <>
          ...
          {/* --- 6.5 REASSURANCE SECTION (client / vendeur) --- */}
          <motion.section
            variants={sectionVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-100px" }}
            className="py-14 md:py-20 bg-gray-50/70"
          >
            <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12">
              <div className="text-center mb-10 md:mb-12">
                <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 tracking-tight mb-2">
                  Achetez et vendez en toute confiance
                </h2>
                <p className="text-gray-500 text-sm md:text-base font-medium max-w-xl mx-auto">
                  Une marketplace pensée pour Cotonou et Abomey-Calavi, avec des paiements sécurisés à chaque étape.
                </p>
              </div>

              <div className="grid md:grid-cols-2 gap-5 md:gap-6">
                {/* Colonne Acheteurs */}
                <div className="bg-white rounded-3xl p-7 md:p-9 border border-gray-100">
                  <h3 className="text-base md:text-lg font-bold text-gray-900 mb-5 tracking-tight">
                    Pour les clients
                  </h3>
                  <ul className="space-y-4">
                    <li className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-gray-600 font-medium">
                        <strong className="text-gray-900">Paiement sécurisé</strong> via Mobile Money, Moov ou carte — vos fonds sont protégés jusqu'à la livraison.
                      </span>
                    </li>
                    <li className="flex items-start gap-3">
                      <CheckCircle2 size={18} className="text-coral-500 shrink-0 mt-0.5" />
                      <span className="text-sm text-gray-600 font-medium">
                        <strong className="text-gray-900">Vendeurs vérifiés</strong> — chaque boutique passe par une validation KYC avant de vendre.
                      </span>
                    </li>
                    <li className="flex items-start gap-3">
                      <CheckCircle2 size={18} className="text-coral-500 shrink-0 mt-0.5" />
                      <span className="text-sm text-gray-600 font-medium">
                        <strong className="text-gray-900">Livraison suivie</strong> — un livreur dédié, une remise confirmée par code, sans surprise.
                      </span>
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-white rounded-3xl p-7 md:p-9 border border-gray-100">
                  ...
                  <ul className="space-y-4">
                    <li className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-gray-600 font-medium">
                        <strong className="text-gray-900">Boutique en quelques minutes</strong> — ouvrez votre vitrine sans frais d'entrée.
                      </span>
                    </li>
                    <li className="flex items-start gap-3">
                      <CheckCircle2 size={18} className="text-teal-600 shrink-0 mt-0.5" />
                      <span className="text-sm text-gray-600 font-medium">
                        <strong className="text-gray-900">Livraison gérée</strong> — Ayiba s'occupe de la logistique via son réseau de livreurs.
                      </span>
                    </li>
                    <li className="flex items-start gap-3">
                      <CheckCircle2 size={18} className="text-teal-600 shrink-0 mt-0.5" />
                      <span className="text-sm text-gray-600 font-medium">
                        <strong className="text-gray-900">Encaissement direct</strong> — vos ventes sont reversées automatiquement sur votre Mobile Money.
                      </span>
                    </li>
                  </ul>
                </div>
          ...
          {/* --- 6.5 BENTO DYNAMIQUE --- */}
          {/* Toutes les cartes viennent de /api/bento (données réelles, cache
              1h côté serveur). N'affiche rien si aucune donnée exploitable
              n'est encore disponible (catalogue trop récent/vide), plutôt
              que de montrer des cartes creuses. */}
          {!bentoLoading && bento && (bento.topCategory || bento.topProduct || bento.categories.length > 0) && (
            <motion.section
              variants={sectionVariants}
              initial="hidden"
              whileInView="visible"
            ...
            >
              <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-12">
                ...
                {/* ===== Desktop : bento mosaïque ===== */}
                <div className="hidden md:grid grid-cols-3 gap-5">
                  {bento.topCategory && (
                    <div
                      className="col-span-2 relative rounded-3xl overflow-hidden p-10 flex flex-col justify-center min-h-[260px] cursor-pointer"
                      style={{ background: "linear-gradient(135deg, #fff1ea, #ffe4e1)" }}
                      onClick={() => router.push(`/catalogue?categorie=${bento.topCategory!.slug}`)}
                    >
                      <span className="inline-block w-fit text-[11px] font-bold uppercase tracking-wider bg-coral-500 text-white px-3 py-1 rounded-full mb-4">
                        Tendance cette semaine
                      </span>
                      <h3 className="text-3xl font-bold text-gray-900 mb-2">{bento.topCategory.nom}</h3>
                      <p className="text-gray-600 text-base mb-5 max-w-sm">
                        {bento.topCategory.nbNouveauxArticles} nouveaux article{bento.topCategory.nbNouveauxArticles > 1 ? "s" : ""} ajouté{bento.topCategory.nbNouveauxArticles > 1 ? "s" : ""}
                        {bento.topCategory.nbVendeurs > 0 && ` par ${bento.topCategory.nbVendeurs} boutique${bento.topCategory.nbVendeurs > 1 ? "s" : ""}`}
                      </p>
                      <span className="w-fit bg-coral-500 text-white text-sm font-bold px-5 py-2.5 rounded-xl">
                        Découvrir
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-5">
                    {bento.newSeller && (
                      <div
                        className="flex-1 rounded-3xl p-6 flex flex-col justify-center bg-teal-50/70 min-h-[118px] cursor-pointer"
                        onClick={() => router.push(`/boutique/${bento.newSeller!.slugOuId}`)}
                      >
                        <span className="text-[11px] font-bold uppercase tracking-wider text-teal-700 mb-1">Nouvelle boutique</span>
                        <h4 className="text-lg font-bold text-gray-900">{bento.newSeller.nomBoutique}</h4>
                        <p className="text-xs text-gray-500">{bento.newSeller.nbArticles} articles disponibles</p>
                      </div>
                    )}
                    {bento.topProduct && (
                      <div
                        className="flex-1 rounded-3xl p-6 flex flex-col justify-center bg-amber-50 min-h-[118px] cursor-pointer"
                        onClick={() => router.push(`/produits/${bento.topProduct!.id}`)}
                      >
                        <span className="text-[11px] font-bold uppercase tracking-wider text-amber-700 mb-1">
                          {bento.topProduct.nbFavoris > 0 ? "Le plus populaire" : "Fraîchement ajouté"}
                        </span>
                        <h4 className="text-lg font-bold text-gray-900 line-clamp-1">{bento.topProduct.nom}</h4>
                        <p className="text-xs text-gray-500">
                          {bento.topProduct.nbFavoris > 0 ? `${bento.topProduct.nbFavoris} favoris cette semaine` : "Nouveau sur Ayiba"}
                        </p>
                      </div>
                    )}
                  </div>

                  {bento.categories.length > 0 && (
                    <div className="col-span-3 grid grid-cols-4 gap-5">
                      {bento.categories.map((cat) => (
                        <div
                          key={cat.slug}
                          className="rounded-2xl bg-white border border-gray-100 p-5 text-center cursor-pointer hover:border-coral-200 transition-colors"
                          onClick={() => router.push(`/catalogue?categorie=${cat.slug}`)}
                        >
                          <div className="w-full h-20 bg-gray-100 rounded-xl mb-3 overflow-hidden flex items-center justify-center text-gray-300 text-xs">
                            {cat.image ? (
                          ...
                          <p className="text-xs text-gray-400">{cat.nbArticles} article{cat.nbArticles > 1 ? "s" : ""}</p>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {/* ===== Mobile : carte pleine largeur + carrousel horizontal ===== */}
                <div className="md:hidden">
                  {bento.topCategory && (
                    <div
                      className="rounded-3xl p-6 mb-5 min-h-[160px] flex flex-col justify-center"
                      style={{ background: "linear-gradient(135deg, #fff1ea, #ffe4e1)" }}
                      onClick={() => router.push(`/catalogue?categorie=${bento.topCategory!.slug}`)}
                    >
                      <span className="inline-block w-fit text-[10px] font-bold uppercase tracking-wider bg-coral-500 text-white px-2.5 py-1 rounded-full mb-3">
                        Tendance cette semaine
                      </span>
                      <h3 className="text-xl font-bold text-gray-900 mb-1">{bento.topCategory.nom}</h3>
                      <p className="text-gray-600 text-xs mb-4">
                        {bento.topCategory.nbNouveauxArticles} nouveaux article{bento.topCategory.nbNouveauxArticles > 1 ? "s" : ""} cette semaine
                      </p>
                      <span className="w-fit bg-coral-500 text-white text-xs font-bold px-4 py-2 rounded-xl">
                        Découvrir
                      </span>
                    </div>
                  )}
                  ...
                  {bento.categories.length > 0 && (
                    <>
                      <div className="text-sm font-bold text-gray-900 mb-3">Catégories populaires</div>
                      <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4" style={{ scrollbarWidth: "none" }}>
                        {bento.categories.map((cat) => (
                          <div
                            key={cat.slug}
                            className="shrink-0 w-28 rounded-2xl bg-white border border-gray-100 p-3 text-center"
                            onClick={() => router.push(`/catalogue?categorie=${cat.slug}`)}
                          >
                            <div className="w-full h-16 bg-gray-100 rounded-lg mb-2 overflow-hidden flex items-center justify-center text-gray-300 text-[10px]">
                              {cat.image ? (
                                <img src={cat.image} alt={cat.nom} className="w-full h-full object-cover" />
                              ) : (
                                "Ayiba"
                            ...
                            <p className="text-xs font-bold text-gray-900 line-clamp-1">{cat.nom}</p>
                            <p className="text-[10px] text-gray-400">{cat.nbArticles} articles</p>
                          </div>
                        ))}
                      </div>
                    </>
                  ...
                  {(bento.newSeller || bento.topProduct) && (
                    <div className="grid grid-cols-2 gap-3 mt-5">
                      {bento.newSeller && (
                        <div
                          className="rounded-2xl p-4 bg-teal-50/70"
                          onClick={() => router.push(`/boutique/${bento.newSeller!.slugOuId}`)}
                        >
                          <span className="text-[9px] font-bold uppercase tracking-wider text-teal-700">Nouveau</span>
                          <p className="text-xs font-bold text-gray-900 mt-1 line-clamp-1">{bento.newSeller.nomBoutique}</p>
                        </div>
                      )}
                      {bento.topProduct && (
                        <div
                          className="rounded-2xl p-4 bg-amber-50"
                          onClick={() => router.push(`/produits/${bento.topProduct!.id}`)}
                        >
                          <span className="text-[9px] font-bold uppercase tracking-wider text-amber-700">
                            {bento.topProduct.nbFavoris > 0 ? "Populaire" : "Nouveau"}
                          </span>
                          <p className="text-xs font-bold text-gray-900 mt-1 line-clamp-1">{bento.topProduct.nom}</p>
                        </div>
                      )}
                    </div>
                  )}
