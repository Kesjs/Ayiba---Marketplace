...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Déconnecte les autres sessions actives du compte (ex. un attaquant
    // connecté ailleurs) — ne bloque pas l'affichage du succès si ça échoue.
    try {
      await fetch("/api/auth/deconnexion-autres-sessions", { method: "POST" });
    } catch {
      // best-effort : le mot de passe est déjà changé, on ne bloque pas l'utilisateur
    }
      ...
      <div className="max-w-sm w-full bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8 max-h-[75dvh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={loading || !!validatePasswordStrength(password) || !confirmPassword}
