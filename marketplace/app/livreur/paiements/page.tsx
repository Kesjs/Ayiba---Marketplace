  ...
  return (
    <div className="p-4 flex items-center justify-between">
      <div className="min-w-0">
        <p className="font-semibold text-gray-900 text-sm">Commande {g.numero}</p>
        <p className="text-xs text-gray-400">
          {g.commune || "—"} · {new Date(g.updated_at).toLocaleDateString("fr-FR")}
        </p>
      </div>
...
export default function LivreurPaiementsPage() {
  const {
    loading, error, gains, retraits,
    soldeDisponible, gainsJour,
  ...
  const [mobileTab, setMobileTab] = useState<"gains" | "retraits">("gains");
  ...
  return (
    <DashboardLayout role="livreur" title="Paiements">
      {error && (
      ...
      {loading ? (
        ...
        >
              ...
              <div className="flex flex-wrap items-center gap-2 mb-5">
                <div className="flex items-center gap-1.5 bg-white/15 backdrop-blur-sm rounded-full px-3 py-1.5">
                  ...
                  <span className="text-xs font-semibold">
                    {gainsJour.toLocaleString("fr-FR")} F aujourd'hui
                  </span>
                </div>
                <div className="flex items-center gap-1.5 bg-white/15 backdrop-blur-sm rounded-full px-3 py-1.5">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            {[
              { icon: Truck, label: "Livraisons payées", value: gains.length, color: "text-teal-600", bg: "bg-teal-50" },
              { icon: ArrowUpFromLine, label: "Retraits", value: retraits.length, color: "text-coral-500", bg: "bg-coral-50" },
              { icon: Zap, label: "Aujourd'hui", value: `${gainsJour.toLocaleString("fr-FR")} F`, color: "text-amber-600", bg: "bg-amber-50" },
          ...
          {/* --- Listes : tabs mobile, côte à côte desktop --- */}
          <div className="lg:hidden bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="flex p-2 gap-2 border-b border-gray-100">
              {(["gains", "retraits"] as const).map((tab) => (
                <button
                  key={tab}
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={mobileTab}
                initial={{ opacity: 0, x: mobileTab === "gains" ? -8 : 8 }}
              ...
              >
                {mobileTab === "gains" ? (
                  gains.length === 0 ? (
                    <p className="text-gray-400 text-center py-12 text-sm">Aucun gain pour l'instant</p>
                  ) : (
                    gains.map((g) => <GainCard key={g.id} g={g} />)
                  )
                ) : retraits.length === 0 ? (
