...
export interface MessagePartner {
  ...
  phone: string | null;
  role: "client" | "vendeur" | "livreur" | null;
}
...
// NOTE : pas de table "conversations" en production (voir schéma réel de
// "messages" : expediteur_id / destinataire_id / commande_id, sans
// conversation_id). Les conversations sont donc reconstituées côté client en
// regroupant les messages par interlocuteur, comme le fait déjà
// useVendeurMessages / useLivreurMessages.
export function useClientMessages() {
  ...
  const fetchPartner = useCallback(async (partnerId: string): Promise<MessagePartner | null> => {
    ...
    return { full_name: row.full_name, avatar_url: row.avatar_url, phone: row.phone, role: row.role };
  }, []);
  ...
  const fetchMessages = useCallback(async () => {
    ...
    const allMessages = ((data as ConversationMessage[]) || []).slice().reverse();
    ...
    if (partnerIds.length > 0) {
      ...
      partnersById = new Map(
        partnersList.map((p) => [
          p.id,
          { full_name: p.full_name, avatar_url: p.avatar_url, phone: p.phone, role: p.role },
        ])
      );
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`messages-realtime-client-${userId}`)
      .on(
        "postgres_changes",
